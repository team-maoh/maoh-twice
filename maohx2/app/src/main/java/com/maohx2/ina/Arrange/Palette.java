...
public class Palette {
    ...
    public Palette(BattleUserInterface _battle_user_interface, Graphic _graphic, int _position_x, int _position_y){
        ...
        for(int i = 0; i < 8; i++){
            palette_elements[i] = new PaletteElement(position_x + (int) (PALETTE_ARRANGE_RADIUS * COS[i]), position_y - (int) (PALETTE_ARRANGE_RADIUS * SIN[i]), i);
        }

        palette_center = new PaletteCenter(position_x, position_y);
    ...
    public void updateSetting(){
        ...
        //指が離れた際のパレットのチェック
        if(battle_user_interface.getTouchState() == Constants.Touch.TouchState.UP && battle_user_interface.getPaletteElement() != null) {
