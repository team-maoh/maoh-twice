...
for backgroundFile in `find $parentDir -mindepth 3 -maxdepth 3 -type f -name "old_firstBackground.*"`; do
  ...
  newBackgroundFile=${backgroundFile%/*}/miningBack.png
  #${backgroundFile##*.}
  ...
  #convert $backgroundFile -fill "#000000" -draw "rectangle $boxX1 $boxY1 $boxX2 $boxY2" -fill "#000000" -draw "circle $circle1PosX $circle1PosY $circle1PosX $((${circle1PosY} + ${circle1R}))" $newBackgroundFile
